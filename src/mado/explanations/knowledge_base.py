...
_KB_BY_CWE: dict[str, KnowledgeEntry] = {
    ...
    "CWE-78": KnowledgeEntry(
        ...
        remediation="Avoid shell invocation when possible; otherwise pass arguments directly and validate input strictly.",
    ...
    "CWE-798": KnowledgeEntry(
        ...
        impact="Secrets can leak through source control, logs, or shared artifacts and are difficult to revoke quickly.",
        remediation="Move secrets to environment variables or a secret manager and rotate exposed values.",
        references=("https://cwe.mitre.org/data/definitions/798.html", "https://owasp.org/www-project-top-ten/"),
    ),
}
...
def lookup_entry(cwe: str | None, rule_id: str | None) -> KnowledgeEntry:
    ...
    return KnowledgeEntry(
        summary="This finding matches a known security pattern, but the local knowledge base has no specific entry yet.",
        ...
        remediation="Review the code path, confirm whether user input reaches the sink, and apply the scanner's recommended fix.",
